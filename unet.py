...
class ConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, dropout_rate=0.2):  # 新增 dropout_rate 参数
...
class Decoder(nn.Module):
    ...
    def __init__(self, name="output", scales=range(1), num_convs=4, init_channels=32, out_channels=1):
        ...
        Args:
            ...
            num_convs (int, optional): [number of up/down levels]. Defaults to 4.
            init_channels (int, optional): [initial number of encoding channels]. Defaults to 32.
            out_channels (int, optional): [number of channels in the output]. Defaults to Gated2Depth-master.
        ...
        self.name = name
        self.up_convs = nn.ModuleList([nn.Sequential(
            nn.ConvTranspose2d(in_channels=self.channels[i], out_channels=self.channels[i + 1], kernel_size=2,
                               stride=2), nn.BatchNorm2d(self.channels[i + 1])) for i in
                                       range(len(self.channels) - 1)])  # [(512->256),(256->128),(128->64),(64->32)]
